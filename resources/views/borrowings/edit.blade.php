            ...
            <div class="bg-white shadow-md rounded-xl p-6">
                ...
                <form action="{{ route('borrowings.update', $borrowing->id) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="item_id" class="w-full border-gray-300 rounded-lg shadow-sm">
                            @foreach($items as $item)
                                <option value="{{ $item->id }}" {{ $borrowing->item_id == $item->id ? 'selected' : '' }}>
                                    {{ $item->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="return_date" value="{{ $borrowing->return_date?->format('Y-m-d') }}"
                            class="w-full border-gray-300 rounded-lg shadow-sm">
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="status" class="w-full border-gray-300 rounded-lg shadow-sm">
                            <option value="borrowed" {{ $borrowing->status == 'borrowed' ? 'selected' : '' }}>Borrowed
                            </option>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="notes" class="w-full border-gray-300 rounded-lg shadow-sm"
                            rows="3">{{ $borrowing->notes }}</textarea>
                    </div>

                    {{-- RUANG PAKAI --}}
                    <div class="mb-4">
                        <label class="block font-medium mb-1">Ruang Pakai</label>
                        <input type="text" name="ruang_pakai" class="w-full border-gray-300 rounded-lg shadow-sm"
                            value="{{ $borrowing->ruang_pakai }}">
                    </div>
