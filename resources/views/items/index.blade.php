                            ...
                            <tbody class="divide-y divide-gray-100 bg-white">
                                @forelse ($items as $item)
                                    <tr class="hover:bg-blue-50/50 transition-colors duration-200">
                                        ...
                                        <td class="px-4 py-4 whitespace-normal break-words">
                                            <span class="inline-flex items-center px-3 py-1.5 bg-gradient-to-r from-gray-100 to-gray-200 rounded-full text-xs font-bold text-gray-700 shadow-sm">
                                                {{ $item->quantity }}
                                            </span>
                                        </td>
                                        ...
                                        @php
                                            $statusStyles = [
                                                'available' => 'bg-gradient-to-r from-green-100 to-green-200 text-green-700 border-green-300',
                                                'borrowed' => 'bg-gradient-to-r from-yellow-100 to-yellow-200 text-yellow-700 border-yellow-300',
                                        ...
                                        <td class="px-4 py-4 whitespace-normal break-words">
                                            <div class="flex flex-wrap gap-1.5">
                                                @foreach ($item->categories as $cat)
                                                    <span class="inline-flex items-center px-3 py-1 text-xs bg-gradient-to-r from-blue-50 to-blue-100 text-blue-700 
                                                                 rounded-full border border-blue-200 font-semibold shadow-sm">
                                                        {{ $cat->name }}
                                                    </span>
                                                @endforeach
                                            </div>
                                        ...
                                        <td class="px-4 py-4 whitespace-normal break-words">
                                            <div class="flex gap-2">
                                                ...
                                                <a href="{{ route('items.edit', $item->id) }}"
                                                    class="group inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gradient-to-r from-amber-500 to-orange-500 text-white 
                                                           hover:from-amber-600 hover:to-orange-600 shadow-md hover:shadow-lg text-xs font-semibold transition-all duration-300">
                                                ...
                                                <button @click="confirmDelete({{ $item->id }}, '{{ $item->name }}')"
                                                    class="group inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-gradient-to-r from-red-500 to-rose-600 text-white 
                                                           hover:from-red-600 hover:to-rose-700 shadow-md hover:shadow-lg text-xs font-semibold transition-all duration-300">
                                                    <svg class="w-3.5 h-3.5 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
