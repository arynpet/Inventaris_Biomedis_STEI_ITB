<x-app-layout>
    ...
    <div class="py-12" x-data="printerPage({{ json_encode($printers->pluck('id')) }})">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- TABLE WRAPPER --}}
            <form id="bulkActionForm" action="{{ route('printers.bulk_action') }}" method="POST">
                @csrf
                        ...
                        <table class="min-w-full divide-y divide-gray-100">
                            ...
                            <tbody class="divide-y divide-gray-100 bg-white">
                                @forelse ($printers as $printer)
                                    <tr class="hover:bg-blue-50/30 transition duration-150" :class="{'bg-blue-50': selectedItems.includes({{ $printer->id }})}">
                                        <td class="px-4 py-4 text-center">
                                            <input type="checkbox" name="selected_ids[]" value="{{ $printer->id }}" 
                                                   @click="toggleItem({{ $printer->id }}, {{ $loop->index }}, $event)"
                                                   :checked="selectedItems.includes({{ $printer->id }})"
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <button type="button" @click="confirmDelete({{ $printer->id }}, '{{ $printer->name }}')" class="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition" title="Hapus">
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/50 backdrop-blur-sm">
            <div @click.away="showModal = false" class="bg-white w-96 rounded-xl shadow-2xl p-6 transform transition-all">
                <h2 class="text-lg font-bold text-gray-800 mb-2">Hapus Printer?</h2>
                <p class="text-gray-500 text-sm mb-6">Yakin ingin menghapus <b class="text-gray-800" x-text="deleteName"></b>?</p>
        ...
        function printerPage(pageIds = []) {
            return {
                ...
                submitBulkAction(type) {
                    if(confirm('Yakin ingin menghapus ' + this.selectedItems.length + ' printer?')) {
